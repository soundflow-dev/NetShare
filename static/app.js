...
let demoT = 0;
const DEMO_DEVICES = [
  { device: "wlan0", type: "wifi", state: "connected", connection: "netshare-wan", role: "wan", ipv4: "192.168.1.42/24", ap_capable: false, band: "5 GHz" },
...
function linkInfoSpan(dev) {
  ...
  if (!txt) return "";
  return `<div class="m link-info" title="${txt}">${txt}</div>`;
}
...
function deviceCard(dev) {
  ...
  const up = dev.state.includes("connected");
  // Wi-Fi sem modo AP não pode partilhar -> opção desativada
  const noAp = isWifi && dev.ap_capable === false;
  const opts = rolesList().map((r) => {
    const block = r.id === "share" && noAp;
    return `<option value="${r.id}" ${dev.role === r.id ? "selected" : ""}` +
      `${block ? " disabled" : ""}>${r.label}` +
      `${block ? " (" + t("role.noAp") + ")" : ""}</option>`;
  }).join("");
  ...
  card.innerHTML = `
    <div class="card-top">
      ...
      <div>
        <div class="card-name">${dev.device}</div>
        <div class="card-sub">${isWifi ? "Wi-Fi" : "Ethernet"}${dev.connection ? " · " + dev.connection : ""}</div>
      </div>
      <span class="pill ${up ? "up" : "down"}">${up ? t("device.active") : t("device.inactive")}</span>
    </div>
    <div class="kv"><span class="k">${t("device.ipv4")}</span><span class="v">${dev.ipv4 || "—"}</span></div>
    <div class="kv"><span class="k">${t("device.role")}</span><span class="v role-tag ${dev.role}">${ROLE_TAG[dev.role]}</span></div>
    <div class="mini">
    ...
    <div class="card-ctrl">
      <label>${t("device.role")}
        <select data-dev="${dev.device}" data-type="${dev.type}">${opts}</select>
      </label>
      ${isWifi ? `<button class="ghost wifi-btn">${t("device.wifiBtn")}</button>` : ""}
    </div>`;
...
async function scanWifi() {
  ...
  } catch (e) { list.innerHTML = `<div class="muted error">${t("err.prefix")}${e.message}</div>`; }
}
...
function wifiRow(net) {
  ...
  row.innerHTML = `
    ...
    <span class="sec">${locked ? "🔒 " + net.security : t("wifi.open")}</span>
    ${action}`;
  if (net.in_use) {
